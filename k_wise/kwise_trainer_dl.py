"""Davidson-Luce (DL) trainer.

This script:
- Loads UltraFeedback dataset
- Keeps ties and models them explicitly
- Trains with 3 seeds
- Uses YAML configuration files for hyperparameter management

Davidson-Luce Model (with ties):
- For ranking π = (B₁, B₂, ..., Bₘ) where Bₜ are tie buckets
- P(π | θ) = ∏ₜ₌₁ᵐ [f(Bₜ) / Σ_{S⊆Rₜ, 1≤|S|≤D} f(S)]
- f(S) = δ_{|S|} * (∏ᵢ∈S exp(θᵢ))^(1/|S|)
- Rₜ = remaining items at stage t
- D = maximum tie size (here D=4)
- δ_{|S|} = tie-weight parameter for subset size |S| (here δ=1 uniformly)
- Loss: -log P(π | θ)
...
def main():
    ...
    # Davidson-Luce specific parameters
    max_tie_size = config.dl.max_tie_size
    tie_penalty = config.dl.tie_penalty
    ...
    class DLCollator:
        ...
        def __call__(self, batch):
            ...
            - Create ordered sequence of buckets B₁ ≻ B₂ ≻ ... ≻ Bₘ
            """
    ...
    # Initialize collator
    collate_fn = DLCollator(tokenizer, max_length)
    ...
    # Implement a Trainer subclass that computes Davidson-Luce loss
    class DLTrainer(Trainer):
        def __init__(self, *args, max_tie_size=3, tie_penalty=1.0, **kwargs):
            super().__init__(*args, **kwargs)
            self.max_tie_size = max_tie_size  # D parameter
            self.tie_penalty = tie_penalty     # δ parameter
        
        def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
            """
            Compute Davidson-Luce log-likelihood loss.
            
            For ranking π = (B₁, B₂, ..., Bₘ) with tie buckets:
            P(π | θ) = ∏ₜ₌₁ᵐ [f(Bₜ) / Σ_{S⊆Rₜ, 1≤|S|≤D} f(S)]
            
            Where:
            - f(S) = δ_{|S|} * (∏ᵢ∈S exp(θᵢ))^(1/|S|)
            - Rₜ = remaining items at stage t
            - D = max_tie_size (maximum allowed tie size)
            - δ = tie parameter
            
            We minimize negative log-likelihood: -log P(π | θ)
            """
            ...
            for prompt_idx, buckets in enumerate(rankings):
                ...
                # Compute Davidson-Luce likelihood for this ranking
                remaining_labels = labels.copy()
                
                for bucket in buckets:
                    # f(Bₜ): worth of the observed bucket
                    bucket_worth = self._compute_f(bucket, prompt_rewards, label_to_idx)
                    
                    # Denominator: sum over all candidate subsets S ⊆ Rₜ with 1 ≤ |S| ≤ D
                    denom = torch.tensor(0.0, device=model.device)
                    
                    # Generate all candidate subsets
                    for size in range(1, min(self.max_tie_size, len(remaining_labels)) + 1):
                        for subset in combinations(remaining_labels, size):
                            subset_worth = self._compute_f(list(subset), prompt_rewards, label_to_idx)
                            denom = denom + subset_worth
                    
                    # Stage probability: P(Bₜ | Rₜ) = f(Bₜ) / Σ f(S)
                    # Log probability: log f(Bₜ) - log Σ f(S)
                    stage_log_prob = torch.log(bucket_worth + 1e-10) - torch.log(denom + 1e-10)
                    
                    total_loss += (-stage_log_prob)
                    
                    # Update remaining items (remove bucket from remaining set)
                    remaining_labels = [label for label in remaining_labels if label not in bucket]
        ...
        def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
            ...
            if prediction_loss_only:
                ...
                model_inputs = {k: v for k, v in inputs.items() if k not in ['rankings']}
                return super().prediction_step(model, model_inputs, prediction_loss_only, ignore_keys)
        
        def _compute_f(self, subset, prompt_rewards, label_to_idx):
            """
            Compute f(S) = δ_{|S|} * (∏ᵢ∈S exp(θᵢ))^(1/|S|)
            
            In log space:
            log f(S) = log δ_{|S|} + (1/|S|) * Σᵢ∈S θᵢ
            
            Then exponentiate to get f(S).
            """
            subset_size = len(subset)
            
            # Tie penalty factor (δ_{|S|} = 1.0 for no penalty)
            tie_penalty_factor = self.tie_penalty if subset_size > 1 else 1.0
            
            # Sum of rewards in subset
            reward_sum = sum(prompt_rewards[label_to_idx[label]] for label in subset)
            
            # f(S) = δ * exp((1/|S|) * Σ θᵢ)
            log_f = torch.log(torch.tensor(tie_penalty_factor, device=prompt_rewards.device)) + (reward_sum / subset_size)
    ...
    logger.info(f'Training complete. Final checkpoint.')
